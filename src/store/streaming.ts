    ...
    set((state) => ({
      nodes: state.nodes.map((n) => {
        ...
        const versions = [...kept, { id: crypto.randomUUID(), question, response, author: 'model' as const,
          ...
          effort: (isAgentModel(modelUsed) && n.data.agentSession?.effort) || undefined,
        }];
        return { ...n, data: { ...withResponseVersions(n.data, versions, versions.length - 1), reasoning: undefined,
          restreaming: undefined, pendingApprovals: undefined, agentTrace: undefined, isLoading: false, tokenCount, generationFailed: failed || undefined,
