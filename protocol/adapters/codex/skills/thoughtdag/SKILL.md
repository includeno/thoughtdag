...
**A. Query history**: when the argument starts with `why`, `find`, `recall` or `status`, this is a query — open no canvas. Pass it verbatim to the ThoughtDAG command line and show its output **as is**: do not read session files yourself, do not summarize or reword (the output is written to be read: Δ is an observed change, ≈ a candidate explanation read from an answer, ⤴ the earlier question a bare reply answered; the `thoughtdag-includeno://…` link ending each line opens that very turn via `open`). Find the command line in this order: ① `command -v thoughtdag` — use it if present; ② else `npx -y @includeno/thoughtdag` (once published on npm); ③ a developer inside the ThoughtDAG repository: `node cli/dist/thoughtdag.mjs` (run `npm run cli:build` first if `cli/dist` is missing). Examples:
