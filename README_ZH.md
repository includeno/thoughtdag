...
大型画布还可以切到一至两层活动邻域、Tree 或 Card 视图，用独立的组织父子/跳转关系、项目标签、自定义类型和组合过滤整理知识；这些导航关系不会进入模型上下文。所有领域修改写入持久化事务，刷新后仍可撤销和重做。
...
本地 CLI 可在用户逐项授权后操作项目、节点、连线、材料、生成、布局与导入导出；删除权限默认关闭。→ [CLI 使用说明](docs/cli_ZH.md)
