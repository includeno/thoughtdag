...
test('merge preserves agent effort through response versions and strips transient agent state', async () => {
  const { responseVersions, withResponseVersions } = await load('/src/lib/response-versions.ts');
  const { stripTransient } = await load('/src/store/index.ts');
  const original = node('agent', { generatedEfforts: ['high'], pendingApprovals: [{ id: 'pending' }], agentTrace: ['running'] });
  const data = withResponseVersions(original.data, responseVersions(original.data));
  assert.deepEqual(data.generatedEfforts, ['high']);
  const [clean] = stripTransient([{ ...original, data, measured: { width: 100, height: 100 } }]);
  assert.equal(clean.measured, undefined);
  assert.equal(clean.data.pendingApprovals, undefined);
  assert.equal(clean.data.agentTrace, undefined);
  assert.equal(clean.data.responseVersions[0].effort, 'high');
});
