...
try {
  ...
  useProjects.setState({
    projects: [{
      id: 'vault-project',
      name: 'Vault recovery',
      sourceSession: { sessionId: 'mirror-session', runner: 'codex', importedCount: 1, tailNodeId: 'current' },
      createdAt: 1,
      updatedAt: 1,
  ...
  assert.equal(backup.projectId, 'vault-project');
  assert.deepEqual(backup.sourceSession, useProjects.getState().projects[0].sourceSession);
